...
description: Conventions for editing the user's personal AgentSkills repository, the canonical source for global skills, agents, and instruction files. Use whenever the user asks to create or modify a personal skill or agent, personal CLAUDE.md or Copilot instructions, or any such file inside AgentSkills. Also use when a link request explicitly concerns AgentSkills or personal skills. Ensures edits target the repository rather than linked global paths, with the required Traditional-Chinese `_Cht` counterpart and commit after content changes.
---
