---
name: git-commit
description: 使用符合 Conventional Commits 規範的格式建立 Commit 訊息，內文採「開頭段落 + 最多 5 項條列」並經換行處理。當使用者要求提交變更、撰寫 Commit 訊息，或需要協助建立具備意義的變更描述與 Ticket 引用時，請使用此技能。當改寫歷史（squash、rebase、amend、fixup、reword）且必須為產生的 Commit 組出訊息時，亦請使用此技能。
---
...
```
- 以 50 個顯示欄寬為目標，最多不超過 72 欄。標題絕不換行。
- Ticket 編號**只**出現在結尾的 `#<no>`。不要同時嵌在 description 裡
  （`feat(refund): PXBOX-27492 … #27492` 是自我重複）。
...
<結尾行：驗證事實>
```
- **最多 5 項**，以「條列項」計算，而非以「行」計算。一個折了三行的
  bullet 仍然只算一項。
- 開頭段落與結尾行都不計入這 5 項。
- **每一行都以 72 個顯示欄寬換行。** 一個 CJK 字元佔 2 個欄寬，因此一行
  繁體中文約可容納 36 個字。
- **空行**用於分隔開頭段落、條列區塊與結尾行。條列項**彼此之間不要**空行。
- **一項只講一件事。** 若某一項需要用 `；`、`——` 或 `，故` 才能把兩個獨立
  陳述串起來，那它就是兩項 — 請拆開，或捨棄較不重要的那半。
- 沒有份量的部分就捨去：只有一項時，直接寫開頭段落並省略條列；當標題已足
  以描述變更時，整個 Body 都省略。
...
8.  **撰寫內文 (選填項目)**：
    ...
    - 遵循 [格式](#格式-format) 所定義的 Body 形狀：開頭段落 → 最多 5 項條列 → 結尾行，每一行都以 72 個顯示欄寬換行。
    - **5 項的上限限制的是「內容量」，不是「密度」。** 絕不可用「把數個陳述塞進一個長 bullet」的方式來滿足它 — 那正是這條上限要防的失敗。若變更真的超過 5 個要點，請把共通主題提升到開頭段落，只保留審查者無法從 diff 推得的那 5 項；若仍然裝不下，代表這個 Commit 做了太多事，應該拆開。
    - 結尾行只承載驗證事實 — 建置結果、測試數量、基底分支。沒有可驗證的內容時就省略。
9.  **破壞性變更偵測**：
    - 偵測變更是否涉及：
        * 移除或修改 API 簽章
        * 資料庫綱要破壞性變更
...
2.  **舊的 Commit 訊息要最後才讀，甚至可以不讀。**
    - 先僅憑淨差異草擬完所有內文項目。之後才可讀取 `git log <base>..HEAD`，且僅用於補回某項變更的*原因*，也僅能用來改寫「已經存在的項目」— 絕不可據此新增任何一項。
    - 在草擬前先去讀舊標題，正是內文帶有「過程感」的直接原因：那些舊標題本身就是過程，而改寫它們是最省力的路徑。不要提早打開它們。
    - **嚴禁**將舊訊息複製、串接或列點寫進新的內文。
3.  **淨效果測試 (net-effect test)** — 寫下每一項內文前都要套用：
    - 一位只針對這個 Commit 執行 `git show` 的讀者，能否看到這項所聲稱的變更？若否，刪除該項。
...
- 保留向後相容層並標記 deprecated，下一個主版本移除。
...
❌ 錯誤 — 把那四個舊標題直接當成條列項，與 `git show` 內容矛盾：
```
