...
```
- **通知使用者前的自我檢查:** 在起草 Task 列表後，驗證每個 Task 項目是否滿足 Design 要求、引用了正確的 Design ID，且不與任何 Design 決策矛盾。在呈現結果前，請自行修復任何遺漏或不一致之處。只有在自我檢查通過後，才通知使用者。
...
- **內容要求**：每個任務的描述必須足夠詳細，使開發者無需反覆查閱 Design 即可實作。必須包含：
    ...
    - **組件**：具體的類別名稱（例如：Handler, Controller, Service）。
    - **方法**：要新增或修改的方法名稱。
    - **實作細節**：逐步的邏輯說明、程式碼模式、或具體的驗證規則。
    - **單元測試**：每個涉及邏輯變動的任務，都必須透過單元測試進行驗證。單元測試與生產程式碼的修改**不強制在同一個任務**，可以**拆分成不同的任務**（例如：T1 寫生產程式碼，T2 寫單元測試），以維持 3 個檔案的限制。
- **任務進程**：功能優先，優化其次。
- **格式**：使用結構化列表而非表格，以利排版與閱讀：
