...
| Q2 | [問題標題] |  | Todo |
```
- 當使用者回答每個 Q 時：在 `## Pre Design Sync` 章節內文的**特定 Q 項目描述中**填寫「結論」（詳細內容），同時更新**進度表**中的「結論」欄位（簡短一、兩句話），並將狀態更新為 `Done` / `Cancel`。
...
- 將使用者的最終決定記錄為「結論」，**並同時更新在特定 Q 項目的內文描述（詳細內容）與進度表（簡短一、兩句話）中**。
