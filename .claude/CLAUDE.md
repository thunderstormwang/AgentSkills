...
## Personal Skills, Agents, and Instructions

**Trigger rule:** before editing, creating, or discussing any of the user's own personal skill,
agent, or instruction file (`CLAUDE.md`, `copilot-instructions.md`) — work that belongs in this
repo (`AgentSkills`), not in some other project — load the `agentskills-conventions` skill
first. It covers where the edit actually belongs (this repo's own clone, never the linked global
copy), the full global-link manifest and how to check/repair it, and the required
Traditional-Chinese `_Cht` translation and commit flow.
