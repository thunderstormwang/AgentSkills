...
**觸發規則：** 每個 session 第一次操作名稱以 `PXBox` 或 `PXEC` 開頭（不分大小寫）的 repo 時，或第一次在對話中提到 B2C 本身（相對於 B2E、3PL 而言，使用者所屬的部門／系統）時，即使當下不在這類 repo 裡，都必須先載入 `b2c-conventions` skill，才能撰寫程式碼、文件、instruction 檔案或進行 commit。該 skill 會覆寫上述檔案語言預設，並收錄後續新增的團隊慣例與隱性知識。
