...
### T1: SQL Script — DB Schema 異動 (Ref: D1)
- **Reference:** [D1]
- **Target:** sql/PXBOX-26599.sql（新建）
- **Implementation Details:**
    - 新建 sql/PXBOX-26599.sql，內容為 ALTER TABLE，對 spu.template_block_groups 新增 3 個欄位：
...
### T2: Entity — TemplateBlockGroupEntity 異動 (Ref: D2)
- **Reference:** [D2]
- **Target:** PXBox.Spu.Domain -> TemplateBlockGroupEntity -> 多個方法
- **Implementation Details:**
    - 新增 3 個 private set 屬性，加 /// <summary> XML doc comment：
        - ProductSelectType (int)
        - PxPayScenarioId (string?)
        - PxGoScenarioId (string?)
    - 新增靜態工廠方法 CreateWithAppier(templateBlockId, sort, pxPayScenarioId, pxGoScenarioId)：
        - 初始化上述 3 欄位
        - ProductSelectType = 2
        - TemplateBlockProducts = new List<TemplateBlockProductEntity>()（空 list）
    ...
    - 修改現有 Update() 方法：切換回手動時，清除 PxPayScenarioId = null、PxGoScenarioId = null、ProductSelectType = 0
    - Unit Test：TemplateBlockGroupEntityTest
        - UpdateWithAppier_ShouldClearTemplateBlockProducts：呼叫後 TemplateBlockProducts 應為空
        - Update_ShouldClearScenarioIds：手動模式呼叫後 ScenarioIds 應為 null
- **Affected Files:**
...
### T3: Entity — TemplateEntity 新增 SetGroupWithAppier (Ref: D2)
- **Reference:** [D2]
- **Target:** PXBox.Spu.Domain -> TemplateEntity -> SetGroupWithAppier
- **Implementation Details:**
    - 新增 SetGroupWithAppier(groupModel) 方法，比照現有 SetGroupWithProduct() 的結構：
        - 若 group 已存在（groupId > 0）：呼叫 group.UpdateWithAppier(pxPayScenarioId, pxGoScenarioId)
        - 若 group 不存在：呼叫 TemplateBlockGroupEntity.CreateWithAppier(...) 並加入 TemplateBlockGroups
        - **不呼叫** SetGroupWithProduct()
    - Unit Test：TemplateEntityTest
...
- **Affected Files:**
    ...
    - src/PXBox.Spu.Test/...TemplateEntityTest.cs（新建 or 已存在則新增 test case）
- **Status:** Todo

---

### T4: EntityConfig — TemplateBlockGroupEntityConfig 新增欄位映射 (Ref: D1, D2)
- **Reference:** [D1, D2]
- **Target:** PXBox.Spu.Infrastructure -> TemplateBlockGroupEntityConfig -> Configure
- **Implementation Details:**
    - 在 Configure() 末尾新增 3 個 Property().HasColumnName() 映射：
        -  => e.ProductSelectType → "product_select_type"
        -  => e.PxPayScenarioId → "pxpay_scenario_id"
        -  => e.PxGoScenarioId → "pxgo_scenario_id"
    - 不加 IsRequired()、HasMaxLength() 等（遵守 EntityConfig 精簡規範）
- **Affected Files:**
...
### T5: Cache — AppierGroupIdCache (Ref: D4)
- **Reference:** [D4]
- **Target:** PXBox.Spu.API -> AppierGroupIdCache（新建）
- **Implementation Details:**
    - 在 src/PXBox.Spu.API/Caches/Template/ 新建 AppierGroupIdCache.cs
    - 新建 DTO（可置於同檔案底部或 Caches/Template/ 目錄下）：
        `csharp
        public class AppierTemplateGroupDto {
            public int TemplateGroupId { get; set; }
            public string PxPayScenarioId { get; set; }
    ...
    - 實作以下方法：
        ...
        - SetAllAsync(List<AppierTemplateGroupDto> groups)：HashSetAsync 以 HashEntry[] 全量覆寫整個 Hash（先 KeyDeleteAsync 再 HashSetAsync）
...
### T6: Cache — AppierTemplateGroupCache (Ref: D4)
- **Reference:** [D4]
- **Target:** PXBox.Spu.API -> AppierTemplateGroupCache（新建）
- **Implementation Details:**
    - 在 src/PXBox.Spu.API/Caches/Template/ 新建 AppierTemplateGroupCache.cs
    - 新建 DTO（可置於同檔案底部）：
        `csharp
        public class AppierProductDto {
            public List<int> ProductIds { get; set; }
    ...
    - Hash tag {appier_template_group_products} 確保所有 key 落同一 Redis Cluster slot（支援 pipeline）
    - 實作以下方法：
        - GetAsync(int templateGroupId, int systemSourceType) → AppierProductDto?：StringGetAsync(key) 後反序列化，再按 systemSourceType 取對應值
        - SetAllAsync(Dictionary<int, Dictionary<int, AppierProductDto>> groups)：使用 IBatch pipeline，對每個 group key 呼叫 StringSetAsync(..., TimeSpan.FromMinutes(30))
- **Affected Files:**
...
### T7: DTO — AppTemplateBlockGroup 新增 ProductSelectType (Ref: D4, D5)
- **Reference:** [D4, D5]
- **Target:** PXBox.Spu.API -> AppTemplateDto.AppTemplateBlockDto.AppTemplateBlockGroup -> 新增欄位
- **Implementation Details:**
    - 在 AppTemplateDto.cs 的 nested class AppTemplateBlockGroup 新增：
        `csharp
        /// <summary>
        /// 選品類型
        /// <br/>0: 手動選品
        ...
        `
    - 確認 RefreshTemplateCahceJob 在建立 AppTemplateBlockGroup 時將 ProductSelectType 從 DB 查詢結果賦值（在 T10 中處理）
- **Affected Files:**
...
### T8: Contract — SaveTemplateCommand 新增 Appier 欄位 (Ref: D3)
- **Reference:** [D3]
- **Target:** PXBox.Spu.API -> SaveTemplateCommand.TemplateBlockGroupModel -> 新增欄位
- **Implementation Details:**
    - 在 TemplateBlockGroupModel 新增 3 個欄位：
        - ProductSelectType (int, default 0)
        - PxPayScenarioId (string?)
        - PxGoScenarioId (string?)
    - 對應 JSON snake_case：product_select_type、pxpay_scenario_id、pxgo_scenario_id（UnderLinePolicy 自動處理，不需手動標記）
- **Affected Files:**
...
### T9: Contract — GetAppGroupProductListQuery 新增欄位 (Ref: D3, Q11)
- **Reference:** [D3]
- **Target:** PXBox.Spu.API -> GetAppGroupProductListQuery -> 新增欄位
- **Implementation Details:**
    - 新增 4 個欄位（Query Parameter，snake_case 由 SnakeCaseQueryValueProviderFactory 處理）：
...
### T10: API Contract 摘要（給前端）(Ref: D3)
- **Reference:** [D3]
- **Target:** 提供給前端工程師的 API 變更摘要（無程式碼變更）
- **Implementation Details:**
    以下摘要供前端工程師參考，可對照 Swagger 確認：

    **① POST /backend/1.0/spu/save_template（後台儲存）**
    - Request Body 中 	emplate_block_groups[] 新增欄位：
        | 欄位 | 型別 | 說明 |
        | :--- | :--- | :--- |
    ...
    **② GET /app/1.0/spu/get_app_group_product_list（前台查詢）**
    - Query Parameters 新增欄位：
        ...
        | 	emplate_type | int | 0 | 0 = 未傳入，跳過封閉賣場判斷 |
        | 	op_category_id | int | 0 | 同上 |
...
### T11: Handler — SaveTemplateCommandHandler Appier 模式支援 (Ref: D2, D5, D6)
- **Reference:** [D2, D5, D6]
- **Target:** PXBox.Spu.API -> SaveTemplateCommandHandler -> CheckInputLogic, SetBlockGroups
- **Implementation Details:**
    - CheckInputLogic()：
        - 遍歷 groups，若 ProductSelectType == 2（Appier）：
            - 跳過「商品數量 ≥ 1」的驗證
            - 改驗證 PxPayScenarioId 與 PxGoScenarioId 皆不可為 null/empty；否則拋 PXECException(ExceptionType.Xxx)
        - ProductSelectType == 0（手動）：現有驗證不動
    - SetBlockGroups()：
        - 遇到 ProductSelectType == 2 改呼叫 	emplate.SetGroupWithAppier(group)
        - 移除（或保留）ProductSelectType == 0 呼叫 	emplate.SetGroupWithProduct(products)（現有邏輯不動）
    - SaveEntitiesAsync() 後不再呼叫 AppierGroupIdCache（移至 EventHandler，見 T13）
    - Unit Test：SaveTemplateCommandHandlerTest
...
### T12: Job — RefreshTemplateCahceJob AppierGroupIdCache 更新 (Ref: D5, D6)
- **Reference:** [D5, D6]
- **Target:** PXBox.Spu.API -> RefreshTemplateCahceJob -> 主執行流程
- **Implementation Details:**
    - 注入 AppierGroupIdCache
    - 在 Job 主流程中，從查詢結果過濾 ProductSelectType == 2 的 groups（這需要 AppTemplateBlockGroup.ProductSelectType 已在 T7 加入）
    - 過濾後的 Appier groups 不寫入 TemplateElasticsearchDto.Groups（避免空 product_marketings 條目）
    - 呼叫 AppierGroupIdCache.SetAllAsync(appierGroups) 全量同步清單（以 AppierTemplateGroupDto 格式）
    - 確認 AppTemplateBlockGroup.ProductSelectType 在寫入 AppTemplateDtoCache 時有從 DB 結果帶入值（mapping 補上）
- **Affected Files:**
...
### T13: EventHandler — RefreshTemplateCahceEventHandler 新增 Appier 快取更新 (Ref: D4, D6)
- **Reference:** [D4, D6]
- **Target:** PXBox.Spu.API -> RefreshTemplateCahceEventHandler -> Handle
- **Implementation Details:**
    - 新增建構子注入：AppierGroupIdCache、IAppierApiService、AppierTemplateGroupCache
    - Handle() 中，現有更新 AppTemplateDtoCache 步驟完成後新增：
        1. 找出 template 中所有 ProductSelectType == 2 的 groups
        2. loop 每個 Appier group：AppierGroupIdCache.AddOrUpdateAsync(group)
        3. loop 每個 Appier group（即時補充，減少空窗）：
            - 呼叫 IAppierApiService.GetRecommendProductsAsync(PxPayScenarioId) → 取得 productIds（max 24）
            - 呼叫 IAppierApiService.GetRecommendProductsAsync(PxGoScenarioId) → 取得 productIds（max 24）
            - 組成 Dictionary<int, AppierProductDto>（key = SystemSourceType）
        4. AppierTemplateGroupCache.SetAllAsync(groups) pipeline 寫入
    - SIT/UAT 環境判斷：比照 RefreshAppierTemplateProductJob 的 hardcode 邏輯（在 T14 中建立後對齊）
- **Affected Files:**
...
### T14: Job — RefreshAppierTemplateProductJob 建立 (Ref: D4, D6)
- **Reference:** [D4, D6]
...
- **Implementation Details:**
    ...
    - Execute() 流程：
        1. GetAllAsync() 取得所有 Appier groups
        2. 若清單為空，直接回傳
        3. 環境判斷：
            - SIT / UAT：使用 hardcode _sitProducts / _uatProducts（PXPay、PXGo 各一份 List\<int\>，比照 CuratingDtoCache 現有做法）
            - Prod：SemaphoreSlim(3) 控制並行數，loop 每個 group 分別呼叫 PxPay / PxGo ScenarioId
        4. AppierTemplateGroupCache.SetAllAsync(groups) pipeline 批次更新
- **Affected Files:**
...
### T15: DI Registration — 新增 Cache 與 Job 到 Startup (Ref: D4, D6)
- **Reference:** [D4, D6]
- **Target:** PXBox.Spu.API -> Startup.cs
- **Implementation Details:**
    - 在相關 Cache 區塊末尾新增（AddSingleton 或 AddScoped，比照現有 AppTemplateDtoCache 的方式）：
        - AppierGroupIdCache
...
### T16: QueryHandler — GetAppGroupProductListQueryHandler Appier 查詢路由 (Ref: D3, D5, D6)
- **Reference:** [D3, D5, D6]
